...
signal read_enable_BRAM, read_enable_reg_2, read_enable_reg_3 : std_logic;
signal r32_o_0, r32_o_1, r32_o_2 : STD_LOGIC_VECTOR(31 DOWNTO 0);--, r32_2_rd, r32_2_inp, r32_3_inp, r32_3_rd, r32_3_rd_aux 
...
--write_enable1_previo <= write_en and chipselect  and      add ;
	...
	r32_0 : reg32 -- REG de ENDEREÇOS da BRAM
	port map (
		clock 	=> clock,
		resetn 	=> resetn,
		WE      => write_enable0,
...
--LOAD  <= r32_o_1 (7 downto 0);
	...
	r32_1 : reg32  -- REG de DADOS da BRAM
	port map (
		clock 	=> clock,
		resetn 	=> resetn,
		WE      => write_enable1,
   ...
   r32_2 : reg32_sc  -- REG de CONTROLE da BRAM 
	port map (
		clock 	=> clock,
		resetn 	=> resetn,
		WE      => write_enable2,
		D 		=> writedata,
		Q 		=> r32_o_2   --readdata_internal
);
...
readdata <= r32_o_0     when read_enable_reg_0    = '1' else 
			...
			r32_o_2     when read_enable_reg_2    = '1' else 
			BRAM_out    when read_enable_BRAM     = '1' else
		                                    (others => 'Z');
...
read_enable_BRAM  <= read_en and chipselect and      add(1)  and      add(0) ;
